    ...
    'endpoints' => [
        ...
        'POST api/claude/followup-rules' => [
            ...
            'frenos'       => [
                ...
                '🔴 Un `estado` que no es un slug real del pipeline es 422 con la lista de los válidos. Es la ASIMETRÍA DELIBERADA con POST claude/followup-rules vs POST claude/followup-templates: allá un estado inexistente es válido a propósito (las plantillas manual_* quedan inertes y disponibles para envío manual), acá no, porque una regla con estado inexistente queda cargada pareciendo que anda y el cron no la levanta nunca.',
                'horas_espera mínimo 1: un 0 convierte el cron de cada dos horas en un martilleo, porque cada corrida vería el tiempo cumplido.',
            ...
            'parametros'   => [
                ...
                ['nombre' => 'reglas[].horas_espera', 'obligatorio' => false, 'validacion' => 'nullable|integer|min:1 — obligatorio al crear la regla', 'que_es' => 'Horas sin respuesta antes de disparar el seguimiento siguiente. Mínimo 1.'],
                ...
                ['nombre' => 'reglas[].activa', 'obligatorio' => false, 'validacion' => 'nullable|boolean — default true al crear', 'que_es' => 'Si el cron levanta esta regla. Apagarla es la forma de sacar un estado del seguimiento automático sin borrar la fila.'],
                ...
                ['nombre' => 'dry_run', 'obligatorio' => false, 'validacion' => 'nullable|boolean — 🔴 DEFAULT true', 'que_es' => 'Sin dry_run=false explícito NO se escribe nada: devuelve el diff por regla y cuántos leads hay en cada estado.'],
        ...
        'POST api/claude/protocol-entries' => [
            ...
            'frenos'       => [
                ...
                '⚠️ No hay índice único en protocol_entries.titulo: si algún día hubiera dos entradas con el mismo título, se actualiza la primera y la segunda queda intacta. No se agregó el índice porque esta misión no lleva migraciones.',
                ...
                'Un título repetido dentro del mismo lote es 422.',
            ...
            'parametros'   => [
                ...
                ['nombre' => 'entradas[].titulo', 'obligatorio' => true, 'validacion' => 'required|string|max:255', 'que_es' => '🔴 Clave natural: es por lo que la carga es idempotente. Es lo que el ABM del panel muestra como identidad de la entrada.'],
                ...
                ['nombre' => 'entradas[].descripcion', 'obligatorio' => false, 'validacion' => 'nullable|string|max:20000 — obligatorio al crear', 'que_es' => 'Cuándo se usa esta entrada. Columna NOT NULL.'],
                ['nombre' => 'entradas[].mensaje_template', 'obligatorio' => false, 'validacion' => 'nullable|string|max:20000 — obligatorio al crear', 'que_es' => 'El texto sugerido. Columna NOT NULL.'],
                ['nombre' => 'entradas[].notas_setter', 'obligatorio' => false, 'validacion' => 'nullable|string|max:20000', 'que_es' => 'Nota para el setter que va a usar la entrada. Mandarla en null la vacía.'],
        ...
        'PATCH api/claude/leads/{id}' => [
            'para_que'     => 'Escribe los campos descriptivos y de agenda de UN lead por lista blanca cerrada: contact_name, company_name, business_type, notes, email, demo_date, demo_start_time, demo_end_time, demo_flexible y meeting_scheduled_at. Es lo que permite AGENDAR una demo desde afuera y guardar el dolor que el lead contó en la conversación (el lugar de eso es `notes`: NO existe columna `dolor`).',
            'escribe'      => true,
            'peligrosidad' => 'baja',
            'frenos'       => [
                ...
                '`phone` NO se escribe desde acá: cambiarlo redirige TODOS los envíos futuros de ese lead a otro número.',
                'dry_run por defecto: si no se pide lo contrario, devuelve el DIFF campo por campo (actual → propuesto) y no escribe absolutamente nada.',
                ...
                'El lead se nombra por id: no hay forma de editarle campos a "los que cumplan un filtro".',
                '🔴 Cambiar `demo_date` resetea recordatorio_demo_enviado, recordatorio_manana_enviado y demo_fin_check_reprogramado_para, por la MISMA definición que usa el panel (App\\Services\\LeadRescheduleFlagsService). Sin ese reset, la demo nueva no recibiría su recordatorio y el check de fin quedaría trabado en un timestamp del horario viejo. El dry_run muestra qué flags se van a resetear.',
                '⚠️ Lo que NO hace, igual que el panel: no mueve el evento de Google Calendar del closer al reagendar (eso es POST admin/lead/{id}/force-calendar-event) ni le avisa al lead por WhatsApp del cambio de horario (DemoScheduledWhatsappService sólo dispara desde el flujo del agente).',
            ],
            'parametros'   => [
                ['nombre' => '{id} (en la ruta)', 'obligatorio' => true, 'validacion' => 'segmento de la URL', 'que_es' => 'Id del lead a editar. Se nombra de a uno: no hay lote.'],
                ...
                ['nombre' => 'campos.contact_name', 'obligatorio' => false, 'validacion' => 'texto, máx 150', 'que_es' => 'Nombre de la persona, para corregir lo que el lead dijo en la conversación.'],
                ['nombre' => 'campos.company_name', 'obligatorio' => false, 'validacion' => 'texto, máx 150', 'que_es' => 'Nombre del negocio.'],
                ['nombre' => 'campos.business_type', 'obligatorio' => false, 'validacion' => 'texto, máx 80', 'que_es' => 'Rubro.'],
                ['nombre' => 'campos.notes', 'obligatorio' => false, 'validacion' => 'texto, máx 20000', 'que_es' => '🔴 El lugar del proyecto para el "dolor detectado": no existe columna `dolor`.'],
                ...
                ['nombre' => 'campos.demo_date', 'obligatorio' => false, 'validacion' => 'texto en formato Y-m-d, fecha real del calendario', 'que_es' => '🔴 Cambiarlo es REAGENDAR: dispara el reset de los flags de recordatorio.'],
                ['nombre' => 'campos.demo_start_time', 'obligatorio' => false, 'validacion' => 'texto HH:MM o HH:MM:SS en 24 horas', 'que_es' => 'Hora de inicio de la demo.'],
                ['nombre' => 'campos.demo_end_time', 'obligatorio' => false, 'validacion' => 'texto HH:MM o HH:MM:SS en 24 horas', 'que_es' => 'Hora de fin de la demo.'],
                ...
                ['nombre' => 'campos.meeting_scheduled_at', 'obligatorio' => false, 'validacion' => 'texto en formato Y-m-d H:i', 'que_es' => 'La videollamada con el closer.'],
                ['nombre' => 'dry_run', 'obligatorio' => false, 'validacion' => 'nullable|boolean — 🔴 DEFAULT true', 'que_es' => 'Sin dry_run=false explícito NO se escribe nada: devuelve el diff campo por campo.'],
