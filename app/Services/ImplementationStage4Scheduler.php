...
use App\Jobs\ProcessImplementationStage4Files;
use Illuminate\Support\Facades\Cache;
use Illuminate\Support\Facades\Log;

/**
 * Programa el procesamiento diferido de archivos de la Etapa 4 con lógica de debounce.
 *
 * Cada vez que llega un archivo (o texto relevante) de la Etapa 4, se reinicia
 * el timer: se incrementa el token de programación y se despacha un nuevo job
 * diferido. Solo el job cuyo token coincida con el almacenado en caché ejecutará
 * el procesamiento; los anteriores se descartan automáticamente.
 */
class ImplementationStage4Scheduler
{
    /** Prefijo de clave de caché por implementación. */
    private const CACHE_KEY_PREFIX = 'impl_stage4_token:';

    /** TTL del token en caché: cubre demoras largas de cola y procesamiento. */
    private const CACHE_TTL_SECONDS = 7200;

    /**
     * Reinicia la espera y programa un job diferido para procesar los archivos acumulados.
     *
     * Llamar este método cada vez que llega un archivo o texto relevante en la Etapa 4.
     * Al llegar un nuevo archivo antes de que el timer expire, el token anterior queda
     * obsoleto y su job será ignorado al ejecutarse.
     *
     * @param int $implementation_id ID de la implementación que recibió un archivo.
     *
     * @return void
     */
    public function schedule_after_file_received(int $implementation_id): void
    {
        // Demora configurable antes de procesar (debounce tras el último archivo recibido).
        $delay = ImplementationSettings::get_file_wait_seconds();
        ...
        ProcessImplementationStage4Files::dispatch($implementation_id, $token)
            ->delay(now()->addSeconds($delay));

        Log::channel('daily')->debug('ImplementationStage4Scheduler: procesamiento diferido programado.', [
