...
/**
 * Avisa por Web Push a los admins suscritos a un lead cuando ese lead manda un mensaje.
 *
 ...
 *
 * RED DE SEGURIDAD: a un admin suscrito al lead que no tenga NINGÚN device registrado en
...
{
    /**
     * Notifica a los admins suscritos al lead que llegó un mensaje nuevo.
     *
    ...
    {
        /* Admins suscritos a ESTE lead (tabla pivot lead_admin_notifications). */
        $admins = $lead->notification_admins()->get();
        ...
        /* Qué admins de los suscritos tienen al menos un device, en una sola consulta: esto
         * corre adentro del webhook de Meta, que es un camino sincrónico y con timeout. */
        $admin_ids_con_device = AdminPushSubscription::whereIn('admin_id', $admins->pluck('id')->all())
            ->distinct()
            ->pluck('admin_id')
            ->map(function ($id) {
                return (int) $id;
        ...
        foreach ($admins as $admin) {
            $tiene_device = in_array((int) $admin->id, $admin_ids_con_device, true);

            if (! $tiene_device) {
                $admin_ids_sin_device[] = (int) $admin->id;
                continue;
            }

            try {
                $this->send_push((int) $admin->id, $nombre, $preview, $data);
