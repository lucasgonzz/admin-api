 ...
 *
 * Por eso el unique `(client_id, fecha, proceso, modelo)` es la pieza central y no un detalle de
 * performance: es lo que hace que volver a pedir un rango REESCRIBA en vez de acumular. Un
 * acumulador que se desincroniza en silencio es exactamente la clase de error que
...
class CreateClientAiTokenUsagesTable extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create('client_ai_token_usages', function (Blueprint $table) {
            ...
            /*
             * 🔴 La clave del upsert idempotente. Nombre explícito y corto porque el que genera
             * Laravel solo queda a cuatro caracteres del techo de 64 de MySQL, y cualquier cambio
             * futuro de nombre de columna lo pasaría.
             */
            $table->unique(['client_id', 'fecha', 'proceso', 'modelo'], 'cli_ai_tok_uso_unico');
